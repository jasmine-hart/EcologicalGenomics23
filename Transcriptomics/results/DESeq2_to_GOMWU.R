...
# how to make files for go enrichment for future
...
# Filter  28 means 75% of samples (so modifying read depth to dial in your analysis)
...
# Optional: Remove the "part1" and "part2" columns from the dataframe saves original 
res_F0_OWvAM_df <- res_F0_OWvAM_df[, !(names(res_F0_OWvAM_df) %in% c("part1", "part2", "part3", "rest"))]

write.table(res_F0_OWvAM_df, file = "res_F0_OWvAM.txt", sep = "\t", row.names = F)   # saves the full original for the records (just as a txt)
...
write.csv(selected_columns_OW, file = "res_F0_OWvAM_LFC.csv", quote = FALSE, row.names = F) # saves the selected columns for GOMWU, quote false bc r makes it a string but it's not actually a string
